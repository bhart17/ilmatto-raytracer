...
void fill_rectangle(rectangle r, uint16_t col) {
    ...
    for (x = r.left; x <= r.right; x++)
        for (y = r.top; y <= r.bottom; y++) write_data16(col);
}
...
void display_char(char c) {
    ...
    for (x = sc; x <= ec; x++) {
        ...
        for (y = sp, mask = 0x01; y <= ep; y++, mask <<= 1)
            write_data16((bits & mask) ? display.foreground
                                       : display.background);
    }
    write_cmd(COLUMN_ADDRESS_SET);
    write_data16(x);
    write_data16(x);
    write_cmd(MEMORY_WRITE);
    for (y = sp; y <= ep; y++) write_data16(display.background);
...
void display_string(char* str) {
    uint8_t i;
    for (i = 0; str[i]; i++) display_char(str[i]);
}
